...
const findJob = function () {
  ...
  if (!inputJob.value && !inputLocation.value) {
    alert("Data not entered. Please, enter one or more parameters");
  } else {
    let jobsFound = [];
    ...
    for (let y = 0; y < jobsFound.length; y++) {
      ulList.innerHTML += `<li>${jobsFound[y].title} | ${jobsFound[y].location}</li>`;
    }
    if (jobsFound.length === 0) {
      alert("The search was unsuccesful");
    }
  }
};

document.querySelector("#searchButton").addEventListener("click", findJob);